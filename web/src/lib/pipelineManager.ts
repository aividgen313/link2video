...
class PipelineManager {
  ...
  private async generateImage(
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (data.success && data.images?.[0]) {
        ...
        console.warn(`[pipeline] Image frame ${i} failed, skipping`);
      }
    }

    if (images.length === 0) throw new Error("Image generation failed for all frames");
    return images;
  }
  ...
  private async generateVideo(
    ...
    const res = await fetch("/api/video", {
      ...
      body: JSON.stringify({
        ...
        mode: "ai",
        model: modelName,
        width: config.videoDimension.width,
        height: config.videoDimension.height,
      }),
      signal,
    });
    ...
    try {
      ...
      const scenePromises = scenes.map(async (scene, index) => {
        ...
        this.update({
          sceneStatuses: {
            ...this.data.sceneStatuses,
            [scene.id]: { ...this.data.sceneStatuses[scene.id], image: true, audio: !!audio || isMusicVideo, imageUrl: firstImg, audioUrl: audio || undefined, duration: actualDuration },
          },
        });
        ...
        if (videoSceneIndices.has(index)) {
          ...
          try {
            aiVideoUrl = await this.generateVideo(scene, config, actualDuration, signal, charPrefix, tier.videoModel);
      ...
      const firstImg = validAssets[0]?.images?.[0];
      ...
      const stitchScenes = validAssets.map((a) => ({
        ...
        transition: "fade",       // Auto-apply smooth fades between clips
        transitionDuration: 0.5,  // 0.5s fade-in/fade-out per clip
      }));
      ...
      await saveToHistory({
        ...
        topic: config.url || "",
        angle: config.scriptData.angle || "",
        thumbnailUrl: (cloudImages[scenes[0]?.id] as any)?.[0] || firstImg,
        quality: config.qualityTier,
        dimensionId: config.videoDimension.id,
        dimensionLabel: config.videoDimension.label,
        totalSeconds: totalSecs,
  ...
  async resumePipeline(projectId: string, config: PipelineConfig, bridge: ContextBridge) {
    ...
    const mergedImages: Record<number, string[]> = { ...config.storyboardImages, ...state.storyboardImages as any };
