    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex border-b border-white/5 bg-black/10">
        {tabs.map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`flex-1 flex flex-col items-center justify-center gap-1.5 py-3 text-[9px] font-black uppercase tracking-widest transition-all relative group ${
              tab === t.id ? "text-primary bg-primary/5" : "text-white/60 hover:text-white/90"
            }`}
          >
            {tab === t.id && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary shadow-[0_0_12px_rgba(37,99,235,0.8)]" />}
            <span className="material-symbols-outlined text-[18px] group-hover:scale-110 transition-transform">{t.icon}</span>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-5">
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <label className="text-[11px] uppercase tracking-wider text-white/70 block font-bold">Visual Prompt</label>
                <button 
                  onClick={() => {
              ...
              <textarea
                ...
                className="w-full h-24 p-3 text-[12px] bg-on-surface/[0.04] border border-on-surface/[0.08] rounded-xl focus:border-primary/50 outline-none transition-all resize-none leading-relaxed"
            ...
            <div className="space-y-3">
              <label className="text-[11px] uppercase tracking-wider text-white/70 block font-bold">Typography</label>
              <button
                onClick={() => setTab("text")}
                className="w-full flex items-center justify-between p-4 rounded-2xl bg-on-surface/[0.04] border border-on-surface/[0.08] hover:border-primary/30 hover:bg-primary/5 transition-all group"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <span className="text-[13px] font-black text-on-surface block">Manage Text Overlays</span>
                    <span className="text-[11px] text-white/40">{selectedScene.overlays.length} active layers</span>
                  </div>
                </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <label className="text-[11px] uppercase tracking-wider text-white/70 block font-bold font-mono">Duration</label>
                <span className="text-[11px] font-mono text-primary bg-primary/10 px-2 py-0.5 rounded-full">{selectedScene.duration}s</span>
              </div>
              <input
                type="range" min="1" max="20" step="0.5"
                value={selectedScene.duration}
                onChange={(e) => updateScene(selectedScene.id, { duration: parseFloat(e.target.value) })}
                className="w-full accent-primary h-1.5 bg-on-surface/[0.08] rounded-full appearance-none cursor-pointer"
              />
            </div>
          </div>
        )}
        ...
        {tab === "style" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div>
              <label className="text-[11px] uppercase tracking-wider text-white/70 block mb-2">Transition</label>
                ...
                {TRANSITIONS.map(t => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-0.5 py-2 px-1 rounded-lg text-[10px] transition-all ${
                      selectedScene.transition === t.value ? "bg-primary/20 text-primary border border-primary/30" : "bg-on-surface/[0.03] text-outline/60"
                    }`}
                  >
            ...
            <div>
              <label className="text-[11px] uppercase tracking-wider text-white/70 block mb-2">Color Filter</label>
                ...
                {FILTERS.map(f => (
                  <button
                    ...
                    className={`py-1.5 px-2 rounded-lg text-[11px] text-left transition-all ${
                      selectedScene.filter === f.value ? "bg-primary/20 text-primary border border-primary/30" : "bg-on-surface/[0.03] text-outline/60"
                    }`}
                  >
                    {f.label}
                  </button>
            ...
            <div>
              <label className="text-[11px] uppercase tracking-wider text-white/70 block mb-2">Camera Motion</label>
              <div className="grid grid-cols-3 gap-1">
                {KEN_BURNS.map(k => (
                  <button
                    key={k.value}
                    ...
                    className={`flex flex-col items-center gap-0.5 py-2 px-1 rounded-lg text-[10px] transition-all ${
                      selectedScene.kenBurns === k.value ? "bg-primary/20 text-primary border border-primary/30" : "bg-on-surface/[0.03] text-outline/60"
                    }`}
                  >
