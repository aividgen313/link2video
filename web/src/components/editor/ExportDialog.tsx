  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="mb-6 flex items-center justify-between glass-subtle p-3 rounded-xl">
          <div>
            <label className="text-xs font-bold text-on-surface">Burn-in Subtitles</label>
            <p className="text-[10px] text-on-surface-variant">Hardcode AI captions into the video</p>
          </div>
          <button
            onClick={() => setBurnSubtitles(!burnSubtitles)}
            className={`w-10 h-6 rounded-full p-1 transition-colors relative ${burnSubtitles ? 'bg-primary' : 'bg-surface-container-highest border border-outline/20'}`}
          >
            <div className={`w-4 h-4 rounded-full bg-white transition-transform ${burnSubtitles ? 'translate-x-4' : 'translate-x-0'}`} />
          </button>
        </div>
