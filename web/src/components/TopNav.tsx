...
export default function TopNav() {
  ...
  const pageSubtitle = (() => {
    switch (pathname) {
      ...
      case "/assets": return "Project management";
    ...
    <header className="h-[56px] md:h-[72px] shrink-0 flex items-center justify-between px-4 md:px-6 topnav-island relative z-10">
      <div className="flex items-center gap-4">
        <div className="flex flex-col">
          <h2 className="font-headline font-black text-xl md:text-2xl tracking-tight text-on-surface leading-tight">
            {pageTitle}
          </h2>
          <div className="flex items-center gap-2 mt-0.5 hidden md:flex">
            <span className="w-1 h-1 rounded-full bg-primary/40 shrink-0" />
            <p className="text-[10px] text-outline font-bold leading-tight tracking-[0.1em] uppercase opacity-80">
              {pageSubtitle}
            </p>
          </div>
        </div>
