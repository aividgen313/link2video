...
/**
 * Delete a file or an entire folder from Supabase Storage.
 * Single file: ?path=projects/123/state.json
 * Folder (recursive): ?path=projects/123&folder=true
 */
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (isFolder) {
      // List all files under this prefix and delete them all
        ...
        for (const file of files) {
          ...
          if (!file.id && file.name) {
            ...
            const { data: subFiles } = await supabase.storage
              .from(BUCKET)
              .list(`${path}/${file.name}`, { limit: 500 });
            if (subFiles) {
              for (const sf of subFiles) {
                if (sf.name) allFiles.push(`${path}/${file.name}/${sf.name}`);
              }
            }
          } else {
      ...
      if (allFiles.length > 0) {
        // Supabase remove supports batch deletion
      ...
      return NextResponse.json({ success: true, path, deletedCount: allFiles.length });
    }
