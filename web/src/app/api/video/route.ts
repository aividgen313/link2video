...
export async function POST(req: NextRequest) {
  try {
    ...
    const qualitySuffix = ", cinematic, photorealistic, 8k, hyperdetailed, professional lighting, high quality video, fluid motion, sharp focus";
    const enhancedVideoPrompt = (prompt || "") + qualitySuffix;
    
    console.log("Video (Pollinations AI mode):", enhancedVideoPrompt.substring(0, 60) + "...");

    const seed = Math.floor(Math.random() * 1000000);
    const clampedDuration = Math.min(Math.max(duration, 2), 10);
    // Use the explicitly requested model if provided, otherwise fallback to premium models
    const VIDEO_MODELS = model ? [model] : ["wan", "seedance-pro", "seedance", "ltx-2"];

    for (let i = 0; i < VIDEO_MODELS.length; i++) {
      const videoModel = VIDEO_MODELS[i];
      const encodedPrompt = encodeURIComponent(enhancedVideoPrompt);
