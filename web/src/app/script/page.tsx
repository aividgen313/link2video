      ...
      {scriptData && (
        <div className="max-w-7xl mx-auto flex flex-col w-full h-[calc(100vh-140px)] min-h-0 overflow-y-auto custom-scrollbar mt-4 pr-1">
          ...
          <div className="grid grid-cols-12 gap-8 pb-10 mt-4">
            ...
            <div className="col-span-12 lg:col-span-7 space-y-6">
                  ...
                  <div className="p-6">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            if (activeScene?.id === scene.id) setActiveScene(newScenes[0]);
                          }}
                        >
                    ...
                    <textarea
                      ...
                      onChange={(e) => {
                        ...
                        setScriptData({ ...scriptData, scenes: newScenes });
                        if (activeScene?.id === scene.id) setActiveScene(newScenes[index]);
                      }}
              ...
              <button
                onClick={() => {
                  ...
                  const newScene: Scene = { id: newId, scene_number: scriptData.scenes.length + 1, duration_estimate_seconds: 8, narration: "", visual_prompt: "" };
                  setScriptData({ ...scriptData, scenes: [...scriptData.scenes, newScene] });
              ...
              <div className="space-y-3 mt-4">
                <div className="flex items-center justify-between px-2">
                  <span className="text-xs text-outline">
                    ...
                    <span className="text-outline/60"> · {scriptData.scenes.length} scenes</span>
                  </span>
                  <span className="text-xs text-outline">
                ...
                {mode !== "music-video" && (
                  <div className="flex gap-3">
                    ...
                    >
                      {isExtending ? "Writing..." : "Continue Writing"}
                    </button>
                    <button
            ...
            {/* Right Side: Visual Prompt Editor */}
            <div className="col-span-12 lg:col-span-5 lg:sticky lg:top-32 h-fit space-y-6">
              {(scriptData.character_identities || (characterProfiles && characterProfiles.length > 0)) && (
                <div className="glass-card rounded-3xl p-6 shadow-xl border-primary/10">
                  <h3 className="font-headline font-extrabold text-sm flex items-center gap-2 text-primary mb-4">
                    ...
                    {scriptData.character_identities ? (
                      Object.entries(scriptData.character_identities).map(([name, desc]) => (
                        <div key={name} className="p-3 rounded-xl bg-surface-container-low border border-outline-variant/10">
                          <input className="text-xs font-bold text-on-surface mb-1 bg-transparent border-none p-0 w-full" value={name} readOnly />
                          <textarea 
                            className="text-[10px] text-outline italic bg-transparent border-none p-0 w-full resize-none h-8"
                            value={desc}
                            onChange={(e) => {
                              const newIdentities = { ...scriptData.character_identities, [name]: e.target.value };
                      ...
                      (scriptData.characterProfiles || characterProfiles).map((p, i) => (
                        <div key={i} className="p-3 rounded-xl bg-surface-container-low border border-outline-variant/10">
                          ...
                          <textarea className="text-[10px] text-outline italic bg-transparent border-none p-0 w-full resize-none h-8" value={p.appearance} readOnly />
                        </div>
                      ))
                    )}
                  </div>
              ...
              <div className="glass-card rounded-3xl p-8 shadow-2xl space-y-8">
                ...
                <div className="relative aspect-video rounded-xl overflow-hidden group">
                  {activeScene && storyboardImages[activeScene.id]?.[0] ? (
                    <img className="w-full h-full object-cover" src={storyboardImages[activeScene.id][0]} alt="Scene Preview" />
                  ) : generatingImages[activeScene?.id || 0] ? (
                    <div className="w-full h-full bg-surface-container-highest flex items-center justify-center">
                      <div className="flex flex-col items-center gap-3">
                        <div className="w-10 h-10 border-3 border-primary/20 border-t-primary rounded-full animate-spin"></div>
                        <span className="text-xs text-outline font-body">Generating...</span>
                      </div>
                    </div>
                  ) : (
                    <div className="w-full h-full bg-surface-container-highest flex items-center justify-center">
                      <div className="flex flex-col items-center gap-2 text-outline">
                        ...
                        <span className="text-xs font-body">No image yet</span>
                      </div>
                    </div>
                  )}
                ...
                <div className="space-y-3">
                  ...
                  <textarea
                    className="w-full bg-surface-container-low border border-outline-variant/20 rounded-xl p-4 font-body text-sm text-on-surface focus:ring-2 focus:ring-primary/40 focus:border-primary/40 resize-none transition-all"
                    rows={4}
                    value={activeScene?.visual_prompt || ""}
                    onChange={(e) => {
                      if (!scriptData || !activeScene) return;
                      const newScenes = scriptData.scenes.map(s => s.id === activeScene.id ? { ...s, visual_prompt: e.target.value } : s);
                      setScriptData({ ...scriptData, scenes: newScenes });
                      setActiveScene({ ...activeScene, visual_prompt: e.target.value });
                    }}
                  />
                </div>

                <button
                  onClick={() => activeScene && generateSceneImage(activeScene)}
                  disabled={!activeScene || generatingImages[activeScene.id]}
                  className="w-full py-3 rounded-xl border border-outline-variant/30 font-body text-sm font-semibold hover:bg-surface-container-lowest transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  <span className={`material-symbols-outlined text-lg ${activeScene && generatingImages[activeScene.id] ? 'animate-spin' : ''}`}>refresh</span>
                  {activeScene && generatingImages[activeScene.id] ? 'Generating...' : 'Regenerate Image'}
                </button>

                <div className="p-4 bg-tertiary/10 rounded-2xl border border-tertiary/20 flex gap-3 text-xs">
                  ...
                  <p className="text-on-surface leading-relaxed">
                    ...
                    Images auto-generate for each scene. Hover over any scene image and click <em>"Regenerate"</em> to get a new one.
                  </p>
                </div>
              </div>
            </div>
