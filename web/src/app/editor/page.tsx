...
// ── Toolbar Icon Button ──
function TBtn({ icon, label, onClick, active, disabled, danger, badge, filled, tooltip }: {
  icon: string; label: string; onClick?: () => void; active?: boolean; disabled?: boolean; danger?: boolean; badge?: string | number; filled?: boolean; tooltip?: string;
}) {
  const { theme: C } = useEditorTheme();
  const btn = (
    <button
      ...
      disabled={disabled}
      title={label}
      className={`relative flex items-center justify-center w-10 h-10 rounded-xl transition-all duration-300 group ${active ? 'shadow-inner' : ''}`}
      style={{
      ...
      onMouseEnter={(e) => { 
        if (!disabled) { 
          e.currentTarget.style.background = active ? `${C.accent}20` : "var(--surface-container-high)"; 
          e.currentTarget.style.color = danger ? C.danger : C.accent;
          e.currentTarget.style.transform = "translateY(-1px)";
        } 
      }}
      onMouseLeave={(e) => { 
        e.currentTarget.style.background = active ? `${C.accent}15` : "transparent"; 
        e.currentTarget.style.color = danger ? C.danger : active ? C.accent : C.textDim;
        e.currentTarget.style.transform = "translateY(0)";
      }}
    >
      <span className="material-symbols-outlined text-[20px] group-hover:scale-110 transition-transform" style={filled ? { fontVariationSettings: "'FILL' 1" } : undefined}>{icon}</span>
      {badge && (
        <span className="absolute -top-1 -right-1 text-white text-[9px] w-4.5 h-4.5 rounded-full flex items-center justify-center font-black shadow-sm" style={{ background: C.accent }}>{badge}</span>
      )}
    </button>
  );
...
function SourceMonitor() {
  ...
  useEffect(() => {
    const handleSwitch = (e: any) => {
      if (e.detail === "media" || e.detail === "scenes") {
        setActiveTab(e.detail);
      }
    };
    window.addEventListener("editor-switch-tab", handleSwitch);
    return () => window.removeEventListener("editor-switch-tab", handleSwitch);
  }, []);
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: C.panel, borderRight: `1px solid ${C.border}` }}>
      ...
      <div className="flex items-center justify-between px-2 flex-shrink-0" style={{ background: C.headerBg, borderBottom: `1px solid ${C.border}`, height: 40 }}>
        ...
        <div className="flex items-center gap-2 pr-3">
          <span className="text-[9px] font-black uppercase tracking-widest text-primary/60">
            {activeTab === "scenes" ? `${scenes.length}` : `${mediaAssets.length}`}
          </span>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-3">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 8 }}>
            {mediaAssets.map(asset => (
              <div
                key={asset.id}
                className="rounded-lg overflow-hidden cursor-grab flex flex-col group/asset"
                style={{ background: C.panelDark, border: `1px solid ${C.border}`, height: 110 }}
                draggable
                onDragStart={(e) => {
                  e.dataTransfer.setData("application/json", JSON.stringify({ assetId: asset.id, type: asset.type, url: asset.url }));
                }}
                title={asset.label}
              >
                <div className="w-full relative flex-1" style={{ background: "rgba(255,255,255,0.02)" }}>
                  {asset.type === "image" ? (
                    asset.url ? (
                      <img src={asset.url || undefined} alt="" className="absolute inset-0 w-full h-full object-cover" draggable={false} />
                    ) : (
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="material-symbols-outlined text-lg" style={{ color: C.accent, opacity: 0.5 }}>image</span>
                      </div>
                    )
                  ) : asset.type === "video" ? (
                    <div className="absolute inset-0 flex items-center justify-center">
                  ...
                  {/* Type badge */}
                  <div className="absolute top-0.5 left-0.5 px-1.5 py-0.5 rounded text-[8px] font-bold uppercase text-white" style={{ background: "rgba(0,0,0,0.7)", zIndex: 10 }}>
          ...
          {mediaAssets.length === 0 && (
            <div className="text-center py-8" style={{ color: C.textMuted }}>
              ...
              <p className="text-[11px]">No project media yet</p>
            </div>
          )}
        </div>
      ) : (
        /* ── Scene Grid ── */
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 gap-2" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", alignContent: "start" }}>
        {scenes.map((scene, index) => {
          ...
          return (
            <div
              ...
              className="cursor-pointer rounded-lg overflow-hidden flex flex-col transition-all hover:scale-[1.02] group/card"
              style={{
                height: 110,
            ...
            >
              <div className="w-full relative flex-1" style={{ background: "rgba(255,255,255,0.02)" }}>
                {scene.imageUrl ? (
                  <img src={scene.imageUrl || undefined} alt="" className="absolute inset-0 w-full h-full object-cover" draggable={false} />
                ) : (
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-3 text-center transition-colors overflow-hidden" style={{ background: "linear-gradient(180deg, transparent, rgba(255,255,255,0.03))" }}>
                    <div className="w-8 h-8 rounded-full flex items-center justify-center mb-1.5 shadow-sm" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.07)" }}>
                      <span className="material-symbols-outlined text-sm" style={{ color: C.accent, fontVariationSettings: scene.aiVideoUrl ? "'FILL' 1" : undefined }}>
                        {scene.aiVideoUrl ? "movie" : "image"}
                      </span>
                    </div>
  ...
  return (
    <div className="p-3 space-y-2.5" style={{ borderBottom: `1px solid ${C.border}` }}>
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-mono" style={{ color: C.textDim }}>{Math.round((trimEnd - trimStart) * 10) / 10}s / {Math.round(selectedScene.duration * 10) / 10}s</span>
      </div>
      <div className="relative h-9 rounded-lg overflow-hidden" style={{ background: "rgba(255,255,255,0.04)" }}>
        {selectedScene.imageUrl && <img src={selectedScene.imageUrl || undefined} className="absolute inset-0 w-full h-full object-cover opacity-25" alt="" />}
      ...
      <button
        onClick={() => {
          const newDur = Math.round(Math.max(2, trimEnd - trimStart) * 10) / 10;
          updateScene(selectedScene.id, { duration: newDur });
        }}
...
function EditorInner() {
  ...
  const {
    scenes, setScenes, tracks, isInitialized, selectedScene, selectedSceneId, setSelectedSceneId,
  ...
  const { statusMessage, showStatus } = useEditorContext();
  ...
  const handleAddText = () => {
    // If no scene is selected, auto-select the first scene
    let target = selectedScene;
  ...
  return (
    <div
      className="editor-container fixed inset-0 z-[100] w-full h-full flex flex-col overflow-hidden"
    ...
    >
                  ...
                  {recentProjects.map((p) => (
                    <div key={p.id} className="relative group">
                      <button
                        ...
                        className="w-full text-left rounded-xl overflow-hidden transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
                        style={{ background: C.panel, border: `1px solid ${C.border}` }}
                      >
                        {/* Thumbnail */}
                        <div className="aspect-video w-full overflow-hidden relative" style={{ background: C.panelDark }}>
                          {p.thumbnailUrl ? (
                            <img src={p.thumbnailUrl || undefined} alt="" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                          ) : (
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          if (confirm(`Delete "${p.title || "Untitled"}" permanently?`)) {
                            deleteFromHistory(p.id);
      ...
      {/* ═══ Header: Top toolbar ═══ */}
      <div className="editor-toolbar flex items-center px-4 gap-2 flex-shrink-0 relative z-[1000]" style={{ background: C.headerBg, borderBottom: `1px solid ${C.border}`, height: 56 }}>
        {/* App logo + back */}
        <button onClick={() => router.push("/script")} className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl transition-all group"
          style={{ color: C.textDim }}
          onMouseEnter={(e) => { e.currentTarget.style.color = C.accent; e.currentTarget.style.background = `${C.accent}10`; }}
        ...
        >
          ...
          <div className="flex flex-col items-start leading-none">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">Editor</span>
            <span className="text-[14px] font-black tracking-tight" style={{ color: C.text }}>Link2Video</span>
          </div>
        </button>
        ...
        {[
          { label: "File", items: [
            { label: "Start New Project", icon: "add_circle", action: () => {
            ...
            { label: "Open Project...", icon: "folder_open", action: () => setShowOpenModal(true), shortcut: "Ctrl+O" },
          ...
          <div key={menu.label} className="relative">
            ...
            {openMenu === menu.label && (
              <>
                <div className="fixed inset-0 z-[490]" onClick={() => setOpenMenu(null)} />
                <div className="absolute top-full left-0 z-[500] py-1 rounded-lg shadow-2xl min-w-[200px]"
        ...
        {/* Workspace switcher */}
        <div className="flex items-center gap-1 mx-4 p-1 rounded-xl" style={{ background: isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.03)", border: `1px solid ${C.border}` }}>
          {([
            ...
            <button
              ...
              style={{
                background: activeWorkspace === ws.id ? C.accent : "transparent",
                color: activeWorkspace === ws.id ? "#fff" : C.textMuted,
                boxShadow: activeWorkspace === ws.id ? `0 4px 12px ${C.accent}40` : "none",
              }}
              title={ws.label}
            >
              <Tooltip content={ws.label}>
        ...
        <div className="flex items-center gap-2 pr-3">
          ...
          <Tooltip content="Export Video (Ctrl+E)">
      ...
      {/* ═══ Toolbar ═══ */}
      <div className="flex items-center px-3 flex-shrink-0 overflow-x-auto custom-scrollbar no-scrollbar" style={{ background: C.headerBg, borderBottom: `1px solid ${C.border}`, height: 44 }}>
        <div className="flex items-center gap-0.5 shrink-0">
          <TBtn icon="save" label="Save Project (Ctrl+S)" onClick={handleSaveProject} active={isSaving} tooltip="Save Project" />
          <TSep />
          <TBtn icon="undo" label="Undo (Ctrl+Z)" onClick={undo} disabled={!canUndo} tooltip="Undo" />
          <TBtn icon="redo" label="Redo (Ctrl+Shift+Z)" onClick={redo} disabled={!canRedo} tooltip="Redo" />
          <TSep />
          <TBtn icon="add" label="Insert Scene" onClick={() => insertScene(selectedScene?.id || null)} tooltip="Insert Scene" />
          ...
          }} disabled={!selectedScene || selectedScene.duration < 2} tooltip="Split Scene" />
          <TBtn icon="content_copy" label="Duplicate (Ctrl+D)" onClick={() => selectedScene && duplicateScene(selectedScene.id)} disabled={!selectedScene} tooltip="Duplicate Scene" />
          ...
          <TSep />
          <TBtn icon="grid_on" label="Safe Zones" onClick={() => setShowSafeZones(!showSafeZones)} active={showSafeZones} tooltip="Toggle Safe Zones" />
          <TBtn icon="straighten" label="Snap (N)" onClick={() => setSnapEnabled(!snapEnabled)} active={snapEnabled} tooltip="Toggle Snap" />
          <TSep />
          <TBtn icon="auto_awesome" label="Add Transitions" onClick={() => applyDefaultTransitions("fade", 0.5)} tooltip="Apply Transitions" />
          <TBtn icon="block" label="Remove Transitions" onClick={() => removeAllTransitions()} tooltip="Clear Transitions" />
        </div>
        ...
        <div className="flex items-center gap-1">
          <TBtn icon="skip_previous" label="Go to Start" onClick={() => { setPlayheadPosition(0); if (scenes.length > 0) setSelectedSceneId(scenes[0].id); }} tooltip="Rewind" />
          ...
          <button
            ...
            className="flex items-center justify-center w-12 h-12 rounded-xl transition-all shadow-lg hover:scale-105 active:scale-95 group relative overflow-hidden"
            style={{ background: isPlaying ? C.danger : C.accent, color: "#fff" }}
          >
            <Tooltip content={isPlaying ? "Pause" : "Play"}>
              <div className="flex items-center justify-center w-full h-full">
                <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity btn-shimmer" />
          ...
          <TBtn icon="skip_next" label="Go to End" onClick={() => { if (scenes.length > 0) { const last = scenes[scenes.length - 1]; setSelectedSceneId(last.id); setPlayheadPosition(getSceneStartTime(last.id)); } }} tooltip="Fast Forward" />
        </div>
        ...
        <div className="flex items-center gap-1 ml-3">
          <TBtn icon="view_sidebar" label="Source" onClick={() => setShowSource(!showSource)} active={showSource} tooltip="Toggle Source Panel" />
          <TBtn icon="tune" label="Properties" onClick={() => setShowProperties(!showProperties)} active={showProperties} tooltip="Toggle Properties Panel" />
          <TBtn icon="fit_screen" label="Reset Layout" onClick={resetLayout} tooltip="Reset UI Layout" />
        </div>
      </div>

      {/* ═══ Main Content: 3-panel layout with resizable borders ═══ */}
      <div className="editor-main flex-1 flex flex-col lg:flex-row min-h-0">
        {/* Left: Source Monitor */}
        {showSource && (
          <>
            <div className="editor-side-panel flex-shrink-0 min-w-[140px] lg:max-w-[400px]" style={{ width: sourceWidth }}>
              <SourceMonitor />
            </div>
        ...
        {showProperties && (
          <>
            ...
            <div className="editor-side-panel flex-shrink-0 flex flex-col min-w-[240px] lg:max-w-[500px]" style={{ width: propsWidth, background: C.panel }}>
              {/* Tabs */}
              <div className="flex items-center px-2 flex-shrink-0" style={{ background: C.headerBg, borderBottom: `1px solid ${C.border}`, height: 34 }}>
      ...
      <div className="flex items-center justify-between px-4 flex-shrink-0" style={{ background: C.headerBg, borderTop: `1px solid ${C.border}`, height: 30 }}>
        <div className="flex items-center gap-3">
          <span className="text-[11px]" style={{ color: C.textDim }}>
            {selectedSceneIds.size > 1 ? `${selectedSceneIds.size} selected` : selectedScene ? `Scene ${selectedScene.orderIndex + 1} — ${Math.round(selectedScene.duration * 10) / 10}s` : "Ready"}
          </span>
        </div>
      ...
      {/* ═══ Global Toast UI ═══ */}
      {statusMessage && (
        <div className="fixed bottom-32 left-1/2 -translate-x-1/2 z-[200] flex items-center gap-3 px-6 py-3 rounded-2xl shadow-2xl animate-in slide-in-from-bottom-4 duration-300"
          style={{ 
            background: statusMessage.type === "success" ? "rgba(16,185,129,0.9)" : "rgba(239,68,68,0.9)",
            color: "#fff",
        ...
        >
          <span className="material-symbols-outlined text-[20px]">
            {statusMessage.type === "success" ? "verified" : "error"}
          </span>
          <span className="text-[13px] font-bold tracking-tight">{statusMessage.text}</span>
        </div>
      )}
    </div>
  );
...
function OpenProjectModal({ onClose, onSelect }: { onClose: () => void; onSelect: (v: VideoHistoryItem) => void }) {
  const { theme: C, isDark } = useEditorTheme();
  const [history, setHistory] = useState<VideoHistoryItem[]>([]);
  useEffect(() => { setHistory(getHistory().sort((a,b) => b.updatedAt - a.updatedAt)); }, []);

  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center backdrop-blur-sm" style={{ background: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      <div className="rounded-xl p-6 w-full max-w-2xl shadow-2xl flex flex-col max-h-[80vh]" style={{ background: isDark ? "#252528" : "#fff", border: `1px solid ${C.border}` }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4 flex-shrink-0">
          <h3 className="text-lg font-bold" style={{ color: C.text }}>Open Project</h3>
          <button onClick={onClose} className="p-1 rounded-lg transition-colors" style={{ color: C.textDim }}
            onMouseEnter={(e) => { e.currentTarget.style.background = isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)"; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; }}
          >
            <span className="material-symbols-outlined text-sm">close</span>
        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-2">
          {history.length === 0 ? (
            <div className="text-center py-8 text-sm opacity-50">No recent projects found.</div>
          ) : (
            history.map(project => (
              <div key={project.id} className="relative group">
                <button
                  onClick={() => onSelect(project)}
                  className="w-full flex items-center gap-4 p-3 rounded-lg border transition-all text-left hover:border-[#E8930C]/50"
                  style={{ borderColor: C.border, background: isDark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)" }}
                >
                  <div className="w-16 h-16 rounded-md bg-black overflow-hidden flex-shrink-0 relative">
                    {project.thumbnailBase64 && !project.thumbnailBase64.startsWith("data:video") ? (
                      <img src={project.thumbnailBase64 || undefined} alt={project.title} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-sm truncate" style={{ color: C.text }}>{project.title || "Untitled Project"}</h4>
                    <div className="flex items-center gap-3 mt-1 text-[11px]" style={{ color: C.textDim }}>
                      <span>{new Date(project.updatedAt).toLocaleDateString()}</span>
                      {project.quality && <span className="uppercase px-1.5 py-0.5 rounded-sm text-[#E8930C]" style={{ background: "rgba(232, 147, 12, 0.1)" }}>{project.quality}</span>}
                    </div>
                  </div>
                  <span className="material-symbols-outlined opacity-0 group-hover:opacity-100 transition-opacity text-[#E8930C] pr-2">arrow_forward</span>
                </button>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    if (confirm(`Delete "${project.title || "Untitled Project"}" permanently?`)) {
                      deleteFromHistory(project.id);
                      setHistory(prev => prev.filter(p => p.id !== project.id));
                    }
                  }}
                  className="absolute right-10 top-1/2 -translate-y-1/2 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-all hover:bg-red-500/10 hover:text-red-500"
                  style={{ color: C.textDim }}
                  title="Delete Project"
                >
