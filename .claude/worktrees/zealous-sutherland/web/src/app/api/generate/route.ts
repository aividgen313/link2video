...
function extractJsonFromText(text: string): string {
  // Strategy 1: Find JSON that starts with {"title" or {"scenes" or {"angle"
  const jsonStartMatch = text.match(/\{[\s\n\r]*"(?:title|scenes|angle|character)/);
  if (jsonStartMatch && jsonStartMatch.index !== undefined) {
    const fromStart = text.substring(jsonStartMatch.index);
    ...
    if (end > 0) return fromStart.substring(0, end + 1);
    // If no matching brace found, return everything and let tryCompleteJson handle it
    return fromStart;
  }
  // Strategy 2: Greedy match (fallback)
  const jsonMatch = text.match(/(\{[\s\S]*\})/);
  return jsonMatch ? jsonMatch[0] : text;
}

function sanitizeJsonString(str: string): string {
  // Fix common issues: unescaped control characters, literal newlines inside strings
  let result = '';
  let inStr = false;
  let esc = false;
...
function parseScriptData(responseText: string): any {
  ...
  // Strip DeepSeek reasoning text that appears before JSON (no <think> tags)
  let jsonStr = extractJsonFromText(cleanText);
  ...
  try {
    ...
    console.warn("Parse attempt 1 failed:", e.message);
    try {
      ...
      console.warn("Parse attempt 2 failed:", e2.message);
      ...
      try {
        ...
        console.error("All 3 parse attempts failed. Last error:", e3.message);
        console.error("JSON length:", jsonStr.length, "chars. Last 200:", jsonStr.substring(jsonStr.length - 200));
        console.error("Extracted JSON attempt (first 1000 chars):", jsonStr.substring(0, 1000));
        throw new Error("Failed to parse AI response as JSON.");
      }
    }
  }

  // If scenes is empty/missing, search deeper for a nested scenes array in the full response
  if (!scriptData.scenes || scriptData.scenes.length === 0) {
    console.warn("Parsed JSON has 0 scenes, searching for nested scenes array...");
    // Check if scenes is under a wrapper key
    for (const key of Object.keys(scriptData)) {
      const val = scriptData[key];
      if (Array.isArray(val) && val.length > 0 && val[0].narration) {
        console.log(`Found scenes under key "${key}" (${val.length} scenes)`);
        scriptData.scenes = val;
        break;
      }
    }
    // Still nothing? Try finding a separate scenes JSON block in the raw text
    if (!scriptData.scenes || scriptData.scenes.length === 0) {
      ...
      if (scenesMatch && scenesMatch.index !== undefined) {
        ...
        const arrStr = extractJsonFromText(fromScenes.replace(/^\s*/, ''));
        try {
          const arr = JSON.parse(tryCompleteJson(repairJson('[' + arrStr.substring(1))));
          if (Array.isArray(arr) && arr.length > 0) {
            console.log(`Extracted ${arr.length} scenes from raw text`);
            scriptData.scenes = arr;
          }
  ...
  scriptData.scenes = scriptData.scenes.map((scene: any, index: number) => ({
    ...
    duration_estimate_seconds: Math.min(scene.duration_estimate_seconds || 5, 8),
  }));
...
- Each scene should be 4-7 seconds with 1-2 sentences of narration — like quick documentary cuts
- PACING IS CRITICAL: Think like a real documentary editor — fast cuts, every line gets a NEW visual
...
- Target approximately ${Math.ceil(durationMinutes * 60 / 5)} scenes for a ${durationMinutes}-minute video (one scene per 5 seconds = fast-paced cuts like a real documentary)
${characterSheet}
...
Format as JSON:
{
  ...
  "scenes": [
    {
      "narration": "1-2 sentences of punchy narration for this quick cut (4-7 seconds).",
...
Generate approximately ${Math.ceil(durationMinutes * 60 / 5)} scenes to fill this duration.
CRITICAL PACING: Each scene = ONE quick cut (4-7 seconds, 1-2 sentences of narration).
Think like a REAL documentary editor: every sentence gets a DIFFERENT visual. Fast cuts. Constant motion.
NEVER write a scene longer than 8 seconds. If you have 3 sentences, that's 3 SEPARATE scenes with 3 different visuals.

Each scene must have:
- narration: 1-2 sentences of punchy voiceover (4-7 seconds of screen time). Short and impactful.
- visual_prompt: Detailed AI image generation prompt describing the exact visual moment (camera angle, lighting, mood, subject). MUST include the FULL physical description of any character who appears. Every scene MUST have a DIFFERENT visual — never repeat the same shot.
- duration_estimate_seconds: 4-7 seconds (NEVER more than 8)
...
Format your response as a JSON object with:
{
  ...
  "scenes": [
    {
      "narration": "1-2 sentences of punchy voiceover. Short, impactful, cinematic. 4-7 seconds of screen time.",
      "visual_prompt": "MUST START with the character's full physical description from character_identities, then camera angle, action, setting, lighting, mood, color palette. MUST be a DIFFERENT visual from the previous scene.",
      "duration_estimate_seconds": 5,
