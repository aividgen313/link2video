...
export default function NotepadPage() {
  ...
  const [synthesizeProgress, setSynthesizeProgress] = useState(0); // 0-100
  ...
  const handleExtract = async () => {
    ...
    setError(null);
    setExtractProgress({ done: 0, total: unextracted.length });

    // Process one source at a time for reliable progress + error isolation
    for (let i = 0; i < unextracted.length; i++) {
      ...
      while (retries >= 0 && !success) {
        try {
          const res = await fetch("/api/notepad/extract", {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              sources: [{ id: source.id, title: source.title, rawContent: source.rawContent }],
            }),
          ...
          if (data.extractions && data.extractions.length > 0) {
            ...
            setNotepadData(prev => ({
              ...
              sources: prev.sources.map(s =>
                s.id === extraction.sourceId ? { ...s, extractedFacts: extraction.facts } : s
              ),
            }));
            success = true;
          }
          ...
          retries--;
          if (retries < 0) {
            // Final fallback: set basic facts so we don't block the pipeline
            console.warn(`Extraction failed for "${source.title}" after retries:`, e.message);
            setNotepadData(prev => ({
              ...prev,
              sources: prev.sources.map(s =>
                s.id === source.id ? {
                  ...
                  extractedFacts: [
                    ...
                    `Content excerpt: ${source.rawContent.substring(0, 200).trim()}${source.rawContent.length > 200 ? "..." : ""}`,
                  ],
                } : s
              ),
            }));
      ...
      setExtractProgress({ done: i + 1, total: unextracted.length });
    }
  ...
  const handleSynthesize = async () => {
    ...
    setSynthesizeProgress(10);
    setError(null);

    // Simulated progress increment while waiting for API
    ...
    try {
      ...
      // Brief delay before resetting progress bar for visual feedback
      setTimeout(() => setSynthesizeProgress(0), 1000);
    }
  };
  ...
  const handleGenerateVideo = () => {
    ...
    setAngle(synthesisResult?.suggestedTitle || synthesisResult?.suggestedAngle || "Documentary Overview");
        ...
        <div className="max-w-2xl mx-auto w-full">
          ...
          {projectsIndex.length === 0 ? (
            <div className="np-card p-8 text-center">
              <span className="material-symbols-outlined text-[48px] mb-3 block" style={{ color: "var(--np-text-tertiary)" }}>note_stack</span>
              <p className="text-[15px] font-medium mb-1" style={{ color: "var(--np-text-secondary)" }}>No saved projects yet</p>
              <p className="text-[13px] mb-4" style={{ color: "var(--np-text-tertiary)" }}>Create a new project to get started</p>
              <button onClick={handleNewProject} className="np-btn-primary px-5 py-2.5 text-[13px] font-semibold rounded-lg inline-flex items-center gap-2">
                <span className="material-symbols-outlined text-[18px]">add</span>
                New Project
              </button>
            </div>
          ...
          <div className="np-card flex flex-col flex-1 overflow-hidden">
            ...
            {sources.length > 0 && (
              <div className="p-3" style={{ borderTop: "1px solid var(--np-divider-light)" }}>
                  ...
                  <div className="mb-2">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[11px] font-medium" style={{ color: "var(--np-text-secondary)" }}>
                        Extracting {extractProgress.done}/{extractProgress.total} sources
                      </span>
                    ...
                    <div className="w-full h-2 rounded-full overflow-hidden" style={{ background: "var(--np-input-bg)" }}>
                      <div
                        className="h-full rounded-full transition-all duration-500 ease-out"
                        style={{ width: `${(extractProgress.done / extractProgress.total) * 100}%`, background: "var(--np-blue)" }}
                      />
                    </div>
                  </div>
                )}
                ...
                >
                  {isExtracting ? (
                    <><div className="w-4 h-4 border-2 border-current/30 border-t-current rounded-full animate-spin" style={{ borderTopColor: "var(--np-blue)" }} />Extracting {extractProgress.done}/{extractProgress.total}...</>
                  ) : allExtracted ? (
        ...
        <div className="lg:w-[260px] xl:w-[280px] flex-shrink-0">
          <div className="np-card p-4 space-y-4">
            ...
            <div className="space-y-1">
              {[
                ...
                },
                {
                  ...
                  sub: isSynthesizing ? `${synthesizeProgress}%` : synthesis ? "Complete" : "Combine all knowledge",
                  done: !!synthesis,
            ...
            <div className="space-y-2.5">
              <div>
                {isSynthesizing && synthesizeProgress > 0 && (
                  <div className="mb-2">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[11px] font-medium" style={{ color: "var(--np-text-secondary)" }}>
                        {synthesizeProgress < 25 ? "Preparing..." : synthesizeProgress < 75 ? "Synthesizing knowledge..." : synthesizeProgress < 100 ? "Parsing results..." : "Done!"}
                      </span>
                      <span className="text-[11px] font-bold tabular-nums" style={{ color: "var(--np-blue)" }}>{synthesizeProgress}%</span>
                    </div>
                    <div className="w-full h-2 rounded-full overflow-hidden" style={{ background: "var(--np-input-bg)" }}>
                      <div
                        className="h-full rounded-full transition-all duration-700 ease-out"
                        style={{ width: `${synthesizeProgress}%`, background: "var(--np-blue)" }}
                      />
                    </div>
                  </div>
                )}
